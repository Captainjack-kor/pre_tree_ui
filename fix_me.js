...
function createTreeView(menu, currentNode) {
  ...
  for(let i=0; i<menu.length; i++){ 
    //menu.length가 children을 만날때 리턴을 못해준다.
    ...
    currentNode.append(li); //root element
  }
  ...
  for (let i of menu){
    ...
    if (i.type === 'item'){
      li.textContent = i.name;
       ...
       input.type = 'checkbox';
       span.textContent = i.name; //음료
       li.append(input,span,ul); // 체크박스+음료+
       createTreeView(i.children, ul); //재귀함수
    }
